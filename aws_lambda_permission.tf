...
resource "aws_lambda_permission" "api_gateway_animal_crossing_villager_get_detail" {
  ...
  source_arn    = "arn:aws:execute-api:${var.aws_region}:${var.aws_account_id}:${aws_api_gateway_rest_api.animal_crossing.id}/*/${aws_api_gateway_method.villager_get.http_method}${aws_api_gateway_resource.villager.path}/*"
