...
class DynamoDBResourceTableGetItemResponse(DynamoDBResponse):
...
class DynamoDBResourceTable(ABC):
    ...
    ) -> DynamoDBResourceTableGetItemResponse:
        raise NotImplementedError
...
def handler(event: Event, _: Context) -> Response:
    ...
    response: DynamoDBResourceTableGetItemResponse = table.get_item(
        Key=event["body"]["key"]
    )
    return Response(
